from __future__ import annotations

from dataclasses import dataclass
from pathlib import Path
from typing import Iterator, Optional


class ContactHypergraphError(RuntimeError):
    pass


@dataclass(frozen=True)
class CanonicalContactRow:
    contigs: list[str]
    contig_weights: Optional[list[float]]
    k_input: int
    k_valid: int
    weight: float


def canonicalize_contact_row(
    *,
    contigs_raw: object,
    contig_weights_raw: object = None,
    k_raw: object = None,
    weight_raw: object = None,
    row_number: Optional[int] = None,
) -> CanonicalContactRow:
    label = f"row {row_number}" if row_number is not None else "contact row"

    if not isinstance(contigs_raw, list):
        raise ContactHypergraphError(f"Invalid contigs type in {label}: expected list, got {type(contigs_raw)!r}")

    nonempty_contigs = [str(c) for c in contigs_raw if str(c)]
    contigs: list[str] = []
    contig_weights: Optional[list[float]] = None

    if contig_weights_raw is not None:
        if not isinstance(contig_weights_raw, list):
            raise ContactHypergraphError(
                f"Invalid contig_weights type in {label}: expected list, got {type(contig_weights_raw)!r}"
            )
        if len(contig_weights_raw) != len(contigs_raw):
            raise ContactHypergraphError(
                f"Mismatched contigs/contig_weights lengths in {label}: "
                f"len(contigs)={len(contigs_raw)} len(contig_weights)={len(contig_weights_raw)}"
            )

        seen: dict[str, int] = {}
        uniq_contigs: list[str] = []
        uniq_w: list[float] = []
        for c, w in zip(contigs_raw, contig_weights_raw, strict=True):
            name = str(c)
            if not name:
                continue
            try:
                ww = float(w)
            except Exception as exc:
                raise ContactHypergraphError(f"Invalid contig_weight in {label}: contig={name!r} value={w!r}") from exc
            if ww < 0.0:
                raise ContactHypergraphError(f"Negative contig_weight in {label}: contig={name!r} w={ww}")
            if ww == 0.0:
                continue
            if name in seen:
                uniq_w[seen[name]] += ww
            else:
                seen[name] = len(uniq_contigs)
                uniq_contigs.append(name)
                uniq_w.append(ww)

        total = float(sum(uniq_w))
        if total > 0.0:
            contigs = uniq_contigs
            contig_weights = [float(x) / total for x in uniq_w]
        else:
            contigs = []
            contig_weights = []
    else:
        seen_names: set[str] = set()
        for name in nonempty_contigs:
            if name in seen_names:
                continue
            seen_names.add(name)
            contigs.append(name)

    try:
        k_input = int(k_raw) if k_raw is not None else len(nonempty_contigs)
    except Exception as exc:
        raise ContactHypergraphError(f"Invalid k value in {label}: {k_raw!r}") from exc

    try:
        weight = float(weight_raw) if weight_raw is not None else 1.0
    except Exception as exc:
        raise ContactHypergraphError(f"Invalid weight value in {label}: {weight_raw!r}") from exc

    return CanonicalContactRow(
        contigs=contigs,
        contig_weights=contig_weights,
        k_input=k_input,
        k_valid=len(contigs),
        weight=weight,
    )


def iter_canonical_contact_rows(
    contacts_path: Path,
    *,
    parquet_batch_size: int = 200_000,
    require_contig_weights: bool = False,
) -> Iterator[CanonicalContactRow]:
    contacts_path = contacts_path.resolve()
    if not contacts_path.exists():
        raise FileNotFoundError(f"Contacts Parquet not found: {contacts_path}")

    try:
        import pyarrow.parquet as pq
    except Exception as exc:  # pragma: no cover
        raise ContactHypergraphError(
            "contact hypergraph construction requires 'pyarrow'. Install it, e.g. pip install pyarrow."
        ) from exc

    parquet = pq.ParquetFile(contacts_path)
    schema = parquet.schema_arrow
    cols = set(schema.names)
    if "contigs" not in cols:
        raise ContactHypergraphError(f"contacts.parquet missing required column 'contigs'. Found: {schema.names}")
    if require_contig_weights and "contig_weights" not in cols:
        raise ContactHypergraphError(
            f"contacts.parquet missing required column 'contig_weights'. Found: {schema.names}"
        )

    has_contig_weights = "contig_weights" in cols
    has_k = "k" in cols
    has_weight = "weight" in cols
    read_cols = (
        ["contigs"]
        + (["contig_weights"] if has_contig_weights else [])
        + (["k"] if has_k else [])
        + (["weight"] if has_weight else [])
    )

    row_number = 0
    for batch in parquet.iter_batches(batch_size=int(parquet_batch_size), columns=read_cols):
        data = batch.to_pydict()
        contigs_list = data["contigs"]
        contig_weights_list = data.get("contig_weights")
        k_list = data.get("k")
        weight_list = data.get("weight")
        for i in range(len(contigs_list)):
            row_number += 1
            yield canonicalize_contact_row(
                contigs_raw=contigs_list[i] or [],
                contig_weights_raw=(contig_weights_list[i] or []) if contig_weights_list is not None else None,
                k_raw=(k_list[i] if k_list is not None else None),
                weight_raw=(weight_list[i] if weight_list is not None else None),
                row_number=row_number,
            )
